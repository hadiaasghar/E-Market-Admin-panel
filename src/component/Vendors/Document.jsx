...
const Document = () => {
  ...
  return (
    <>
      {/* <div><DocUparbar /></div> */}
      <TitleHead title={"Vendor Document Details"} desc={"Vendor Document Details"} desc2={"> vendors "} link={"/vendors"}/>
      <div className="p-4">
          ...
          <div className="bg-[#F7F7F7] mt-2 flex items-center justify-center border-b border-gray-300">
            <div className='bg-[#267FFF] w-full sm:w-3/4 md:w-1/2 lg:w-1/3 py-2 flex items-center justify-center mx-auto rounded'>
              <p className='text-center text-white text-lg'>
                iJul Store's Vendor Document Details
              </p>
            </div>
          </div>
          <div className="p-4">
            <div className="flex flex-col sm:flex-row sm:justify-between py-2">
              ...
              <div>
                <label htmlFor="search" className="text-gray-800 px-4">Search: </label>
                <input
                  id="search"
                  type="text"
                  value={search}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredData.map((item, index) => (
                    <tr key={index} className='text-sm'>
                      <td className="px-2 py-2 whitespace-nowrap border border-gray-300">{item.name}</td>
                      <td className="px-2 py-2 whitespace-nowrap border border-gray-300">
                        <span className={`p-2 inline-flex leading-5 font-semibold rounded-lg ${item.status === 'Approved' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                          {item.status}
                        </span>
                      </td>
